...
outname = glue(
  "Receptors-{cell_rec}-vs-Ligands-{cell_lig}-top-{howmany}-interactions-ranking-{ranking}-{confidence_level}-confidence-multiple-group.tiff"
)
...
final = df %>% select(
  ...
  mutate(
    V1 = "Receptor",
    V2 = "Ligand",
    Rank = case_when(ranking == "Ranking1" ~ Ranking1,
...
if (!("any_cell" %in% cell_lig)) {
  final = final %>% filter(`Cell type.lig` %in% cell_lig)
}
if (!("any_cell" %in% cell_rec)) {
  final = final %>% filter(`Cell type.rec` %in% cell_rec)
}
